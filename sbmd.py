...
def eval(data, render=False):
    ...
    agent = SBMD([8, args["hnodes"], 4], args["prate"], 0.01, args["seed"], False)
    agent.set_hrules(x)
    for i in range(100):
...
def experiment_launcher(config):
    ...
    ps = config["ps"]
    print(config)
    os.makedirs("./results_SBMD_ll/", exist_ok=True)
    os.makedirs("./results_SBMD_ll/" + str(ps[0]), exist_ok=True)
    os.makedirs("./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate), exist_ok=True)
    os.makedirs("./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate) + "/" + str(hnodes), exist_ok=True)
    os.makedirs("./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate) + "/" + str(hnodes) + "/" + str(seed),
                exist_ok=True)

    fka = SBMD([8, hnodes, 4], prate, 0.01, seed, True)
    ...
    while gen <= args["max_generations"]:
        ...
        with open("./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate) + "/" + str(hnodes) + "/" + str(
                seed) + "/best_" + str(
            gen) + ".pkl", "wb") as f:
    ...
    with open("./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate) + "/" + str(hnodes) + "/" + str(seed) + "/" + str(
            best_fitness) + ".pkl", "wb") as f:
        pickle.dump(best_guy, f)
    with open("./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate) + "/" + str(hnodes) + "/" + str(seed) + "/log.txt",
              "w") as f:
        for l in logs:
            f.write(l + "\n")
        ...
        for ps in [[1],[10],[20]]:
            for prate in [0,80,90]:
                for hnodes in [5,20,90]:
                    dir = "./results_SBMD_ll/" + str(ps[0]) + "/" + str(prate) + "/" + str(hnodes) + "/" + str(seed) + "/"
                    if not chs(dir):
