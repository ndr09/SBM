...
class ND_SBM(HNN):
    ...
    def sanitize_weights(self):
        # clean impossible connections
        #   outputs to other nodes
        self.weights[self.nodes[0] + self.nodes[1]:self.tns, :] = 0
        #   incoming edges to input
        self.weights[:self.tns, :self.nodes[0]] = 0
    ...
    def update_weights_node(self, node):
        # for i in range(self.tns-self.nodes[-1]):
        ...
        indices = np.arange(self.tns - self.nodes[-1])

        # Mask to exclude pruned synapses
        mask = np.array([(i, node) not in self.pruned_synapses for i in indices])

        # Select valid indices based on the mask
        valid_indices = indices[mask]

        # Vectorized calculations for dw on valid indices
        hrules_valid = self.hrules[valid_indices]  # Select rows of hrules for valid indices
        activations_valid = self.activations[valid_indices]  # Select activations for valid indices

        # Calculate dw for each valid index i
        dw = (
                hrules_valid[:, 2] * self.hrules[node][2] * activations_valid * self.activations[node] +  # both
                self.hrules[node][1] * self.activations[node] +  # post
                hrules_valid[:, 0] * activations_valid +  # pre
                self.hrules[node][3] * hrules_valid[:, 3]  # other term
        )

        # Calculate weight updates
        weight_updates = 0.5 * (hrules_valid[:, 4] + self.hrules[node][4]) * dw

        # Update weights in a vectorized manner
        self.weights[valid_indices, node] += weight_updates
        self.sanitize_weights()

    def mybl(self, arr, target):
    ...
    def _isa(self, l: list, v: int):
        ...
        left, right = 0, len(l)
        # c = 0
        # assert self.is_sorted(l), 'e che cazzo'
        while left < right:
            mid = (left + right) // 2
            if l[mid] > v:
                left = mid + 1
        ...
        # print('executed ', c)
        l.insert(left, v)  # Maintain ascending order with efficient insertion
        return l, left

    def _isa2(self, l: list, v: int):
        # Find the index where the element should be inserted
        index = 0
        # self.ecc = 0
        assert self.is_sorted(l), l
        while index < len(l) and l[index] > v:
            self.ecc += 1
            index += 1
        # Insert the element at the found index
...
class NDEP_SBM(ND_SBM):
    def __init__(self, nodes, prune_ratio=0, seed=None, random=True):
        ...
        self.prune_ratio = np.zeros((sum(self.nodes[1:]),2)) # pr 0 ps 1
        self.prune_flag = set()
        self.nparams += sum(self.nodes[1:])*2
        self.fire_order_ind = []
        self.act_counter = 0
        self.tmp_dict = {tn:-1 for tn in range(self.nodes[0], self.tns)}
    ...
    def prune_weights_node(self, node):
        ...
        thr = np.percentile(wsToThr, self.prune_ratio[node-self.nodes[0]][0]*100)
        self.prune_flag.add(node)
        ind = wsToThr <= thr
        self.weights[ind,node] = 0
        self.pruned_synapses.update({(i, node) for i in np.where(ind)[0]})
    ...
    def activate(self, inputs, update=True):
        ...
        for i in range(len(inputs)):
            self.activations[i] = np.tanh(inputs[i])
        if self.first_rodeo:
            self.fire_order_ind = list(range(self.nodes[0], self.tns))
            np.random.shuffle(self.fire_order_ind)
            s = time.time()
            u = []
            p = []
            for node in self.fire_order_ind:
                self._fired.add(node)
                self.activations[node] = np.tanh(np.dot(self.activations, self.weights[:, node]))
                ###### REMEMBER THIS!
                us = time.time()
                if update and not node in self.prune_flag:
                    self.update_weights_node(node)
                u.append(time.time()-us)
                pts = time.time()
                if self.prune_ratio[node-self.nodes[0]][1] == self.act_counter:
                    self.prune_weights_node(node)
                p.append(time.time()-pts)
            # print('act + prun',sum(u), sum(p))
                ###### LOOK UP !!!!!!
            self.first_rodeo = False
        else:
            s = time.time()
            ...
            for node in range(self.nodes[0], self.tns):
                nw, ni, clean = self._argmax(self.weights[:, node][:self.nodes[0]], fire_order_dis, self.fire_order_ind)
                if not nw == ni == clean == -1:  # if all are -1 it means that the node is not reachable and it will not considered
                    fire_order_dis, index = self._isa(fire_order_dis, nw)
                    self.fire_order_ind.insert(index, node)
                # else:
                #     print("this node is not linked    "+str(node))
            # print(fire_order_ind)
            # print(fire_order_ind)
            ...
            par = []
            upt = []
            for i in range(len(self.fire_order_ind)):
                node = self.fire_order_ind[i]
                self._fired.add(node)
                self.activations[node] = np.tanh(np.dot(self.activations, self.weights[:, node]))
                ###### REMEMBER THIS!
                up = time.time()
                ...
                if update and not node in self.prune_flag:
                    self.update_weights_node(node)
                upt.append(time.time()-up)

                if self.prune_ratio[node-self.nodes[0]][1] == self.act_counter:
                    self.prune_weights_node(node)
                ###### LOOK UP !!!!!!
                porder = defaultdict(lambda:-1)
                porder.update({tn:npos for tn, npos in zip(self.fire_order_ind, range(len(self.fire_order_ind)))})
                # now I have to check if the order must be changed
                part = time.time()
                for outnode in range(self.nodes[0], self.tns):
                    # If at this iteration the outnode has already been fired skip it.
                    ...
                    if outnode not in self._fired:
                        outc = self.weights[node, outnode]
                        outnode_index = porder[outnode] #self._gie(self.fire_order_ind, outnode)
                        ...
                        if outnode_index != -1:  # this can happen if the node is not reachable, well I still have to had the code to handle this. For now it will not happen
                            ...
                            if wnode - woutc < outc:  # yes I can reach it paying less, well more, but I mean faster, hopefully
                                ...
                                # Hence, the one remaining is the last so no need to change its position
                                if i + 2 < len(fire_order_dis):
                                    self.fire_order_ind.pop(outnode_index)
                                    fire_order_dis.pop(outnode_index)
                                    tmp_dis, newIndex = self._isa(fire_order_dis[i + 1:], wnode - woutc + outc)
                                    tmp_ind = self.fire_order_ind[
                                              i + 1:]  # new index refers to the position in the smaller vector
                                    tmp_ind.insert(newIndex, outnode)
                                    fire_order_dis = fire_order_dis[:i + 1] + tmp_dis
                                    self.fire_order_ind = self.fire_order_ind[:i + 1] + tmp_ind
                                    tmpo = {a:(b+i+1) for a,b in zip(tmp_ind, range(len(tmp_ind)))}
                                    porder.update(tmpo)
                par.append(time.time()-part)
            p2 = time.time()-s
            # print('\t 2', sum(par), sum(upt), p2, 100*((sum(par)+sum(upt))/p2))
