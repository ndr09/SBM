...
class Network(nn.Module):
    def __init__(self, params):
        ...
        self.activ = F.tanh
        if params['type'] == 'rnn':
            self.i2h = torch.nn.Linear(TOTALNBINPUTS, params['hs'])
            self.w =  torch.nn.Parameter((.01 * torch.rand(params['hs'], params['hs'])), requires_grad=True)
        elif params['type'] == 'modplast':
            self.i2h = torch.nn.Linear(TOTALNBINPUTS, params['hs'])
            self.w =  torch.nn.Parameter((.01 * torch.t(torch.rand(params['hs'], params['hs']))), requires_grad=True) 
            self.alpha =  torch.nn.Parameter((.01 * torch.t(torch.rand(params['hs']))), requires_grad=True)
            self.h2DA = torch.nn.Linear(params['hs'], NBDA)
        elif params['type'] == 'plastic' :
            self.i2h = torch.nn.Linear(TOTALNBINPUTS, params['hs'])
            self.w =  torch.nn.Parameter((.01 * torch.rand(params['hs'], params['hs'])), requires_grad=True)
            self.alpha =  torch.nn.Parameter((.01 * torch.rand(params['hs'])), requires_grad=True)
            self.eta = torch.nn.Parameter((.01 * torch.ones(1)), requires_grad=True)  # Everyone has the same eta
        elif params['type'] == 'modul' or params['type'] == 'modul2':
            self.i2h = torch.nn.Linear(TOTALNBINPUTS, params['hs'])
            self.w =  torch.nn.Parameter((.01 * torch.rand(params['hs'], params['hs'])), requires_grad=True)
            self.alpha =  torch.nn.Parameter((.01 * torch.rand(params['hs'])), requires_grad=True)
            self.etaet = torch.nn.Parameter((.01 * torch.ones(1)), requires_grad=True)  # Everyone has the same etaet
            self.h2DA = torch.nn.Linear(params['hs'], NBDA)
        else:
            raise ValueError("Which network type?")
        self.h2o = torch.nn.Linear(params['hs'], NBACTIONS)
...
def train(paramdict):
    ...
    for numiter in pbar:
        ...
        for numstep in range(params['eplen']):
            ...
            pbar.set_description("Reward: %f" % sumreward.mean())
            pbar.set_postfix(dist=dist)
            pbar.update(1)
            # if PRINTTRACE:
        ...
        R = Variable(torch.zeros(BATCHSIZE), requires_grad=False)
        gammaR = params['gr']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wp", type=float, help="penalty for hitting walls", default=.00)
    ...
    parser.add_argument("--msize", type=int, help="size of the maze; must be odd", default=13)
    ...
    parser.add_argument("--addpw", type=int, help="are plastic weights purely additive (1) or forgetting (0) ?", default=3)
    ...
    parser.add_argument("--nbiter", type=int, help="number of learning cycles", default=200002)
