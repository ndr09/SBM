...
import gym
from spiking_network import SNN, LifNeuron
import numpy as np
import functools
...
def rate_code(inputs, n_timestamps):
    spikes = []
    
    #data standardization
    inputs[0] = (inputs[0]-(-1.5))/3
    inputs[1] = (inputs[1]-(-1.5))/3
    inputs[2] = (inputs[2]-(-5))/10
    inputs[3] = (inputs[3]-(-5))/10
    inputs[4] = (inputs[4]-(-3.14))/6.28
    inputs[5] = (inputs[5]-(-5))/10
    #inputs[6] = (inputs[6]-(0))/1
    #inputs[7] = (inputs[7]-(0))/1
    
    for i in range(len(inputs)):
        n_spikes = round(inputs[i]*n_timestamps)
        spikes.append([1] * n_spikes + [0] * (n_timestamps - n_spikes))
        random.shuffle(spikes[i])

    outputs = list(zip(*spikes))

    return [list(riga) for riga in outputs]


def eval(x, render=False):
    cumulative_rewards = []
    task = gym.make("LunarLander-v2")
    agent = SNN([8, 5, 4], 20, 1, 0.5)
    agent.set_weights(x)
    for i in range(100):
        ...
        counter = 0
        while not done:
            output = agent.activate(rate_code(obs, 25))
...
def plot_fitnesses(average_scores_history, best_scores_history):
    ...
    plt.savefig(f'./LunarLander_SNN_PruningIteration#{len(average_scores_history)-1}.png')


if __name__ == "__main__":
    args = {}
    fka = SNN([8, 5, 4], 20, 1, 0.5)
    rng = np.random.default_rng()

    args["num_vars"] = fka.nweights
    args["max_total_generations"] = 300
    args["max_partial_generations"] = 30
    args["sigma"] = 1.0
    args["pop_size"] = 4
    args["num_offspring"] = 20
    args["pop_init_range"] = [-1, 1]
