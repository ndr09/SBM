...
def eval(x, render=False):
    ...
    for i in range(100):
        ...
        counter = 0
        while not done:
            output = agent.activate(rate_code(inputs_resizing(obs, x), 30))
...
def cart_pole_SNN_ratecode(baseFolder):
        
    for tests in range(5): # esegue 5 test ed effettua il grafico con medie e error bars

        print("TEST " + str(tests) + " CART POLE SNN RATECODE")
        ...
        save_scores(average_scores_history, best_scores_history, tests, baseFolder+"/cart_pole_SNN_ratecode_")

        with open("./"+baseFolder+"/nn_cartpole_SNN_ratecode"+str(best_fitness)+".pkl", "wb") as f:
            pickle.dump(best_guy, f)

    plot_average_fitnesses([baseFolder+'/cart_pole_SNN_ratecode_scores_run_0.txt',
                            baseFolder+'/cart_pole_SNN_ratecode_scores_run_1.txt',
                            baseFolder+'/cart_pole_SNN_ratecode_scores_run_2.txt',
                            baseFolder+'/cart_pole_SNN_ratecode_scores_run_3.txt',
                            baseFolder+'/cart_pole_SNN_ratecode_scores_run_4.txt'], baseFolder+"/CartPole_SNN_ratecode.png")